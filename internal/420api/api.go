...
//
// https://github.com/420integrated/go-420coin/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid 420coin signature (V is not 27 or 28)")
	}
...
// NewPublicBlockChainAPI creates a new 420coin blockchain API.
...
func (args *CallArgs) ToMessage(globalSmokeCap uint64) types.Message {
	...
	msg := types.NewMessage(addr, args.To, 0, value, smoke, smokePrice, data, false)
	return msg
}
...
func DoCall(ctx context.Context, b Backend, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides map[common.Address]account, vmCfg vm.Config, timeout time.Duration, globalSmokeCap uint64) (*core.ExecutionResult, error) {
	...
	// Setup the smoke pool (also for unmetered requests)
...
func (args *SendTxArgs) toTransaction() *types.Transaction {
	...
	return types.NewTransaction(uint64(*args.Nonce), *args.To, (*big.Int)(args.Value), uint64(*args.Smoke), (*big.Int)(args.SmokePrice), input)
}
...
// Version returns the current 420coin protocol version.
...
func checkTxFee(smokePrice *big.Int, smoke uint64, cap float64) error {
	...
	feeFourtwenty := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(smokePrice, new(big.Int).SetUint64(smoke))), new(big.Float).SetInt(big.NewInt(params.Fourtwenty)))
