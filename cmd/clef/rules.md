...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 420coin` in value to maximum `1 420coin` per 24h period
* I want to auto-approve transaction to contract `420AlarmClock` with `data`=`0xdeadbeef`, if `value=0`, `smoke < 44k` and `smokePrice < 40Gmarley`
...
implemented for `g420`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
