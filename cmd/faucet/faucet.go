...
var (
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for 420coin and the faucet")
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s 420coins", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // 420coin address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an 420coin light client.
type faucet struct {
...
// close terminates the 420coin connection and tears down the faucet.
...
// apiHandler handles requests for 420coin grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the 420coin address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/420integrated/go-420coin/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the uniqueness identifier (user id/username), username, avatar URL and 420coin address to fund on success.
func authTwitter(url string, token string) (string, string, string, common.Address, error) {
	...
	// Twitter's API isn't particularily friendly with direct links.
	// It is restricted to 300 queries per 15 minutes with an app api key.
	...
	// and scrape it for the 420coin address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No 420coin address found to fund")
	}
	var avatar string
...
func authTwitterWithToken(tweetID string, token string) (string, string, string, common.Address, error) {
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No 420coin address discovered in which to fund")
	}
...
// returning the username, avatar URL and 420coin address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the 420coin address and profile URL.
	res, err := http.Get(url)
	if err != nil {
		return "", "", common.Address{}, err
	}
...
// authNoAuth tries to interpret a faucet request as a plain 420coin address,
