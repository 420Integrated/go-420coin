...
func TestAccountNew(t *testing.T) {
	g420 := runG420(t, "account", "new", "--lightkdf")
	defer g420.ExpectExit()
	g420.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	g420 := runG420(t, "account", "import", keyfile, "-password", passwordFile)
	defer g420.ExpectExit()
	g420.Expect(expected)
}
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(g420.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(g420.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	g420.SetTemplateFunc("keypath", func(file string) string {
