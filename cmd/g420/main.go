...
// g420 is the official command-line client for 420coin.
package main
...
const (
	clientIdentifier = "g420" // Client identifier to advertise over the network
)
...
var (
	...
	nodeFlags = []cli.Flag{
		...
		utils.LegacyMinerSmokeTargetFlag,
		utils.MinerSmokeLimitFlag,
		utils.MinerSmokePriceFlag,
		utils.LegacyMinerSmokePriceFlag,
		utils.Miner420coinbaseFlag,
		utils.LegacyMiner420coinbaseFlag,
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.RinkebyFlag.Name):
		log.Info("Starting G420 on Rinkeby testnet...")

	case ctx.GlobalIsSet(utils.GoerliFlag.Name):
		log.Info("Starting G420 on Görli testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting G420 in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting G420 on 420coin mainnet...")
	}
...
// g420 is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func g420(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend 420api.Backend) {
	...
	// Create a client to interact with local g420 node.
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full 420coin node is running
