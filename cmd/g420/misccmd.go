...
var (
	VersionCheckUrlFlag = cli.StringFlag{
		...
		Value: "https://420integrated.com/420coin-overview/g420/g420-vulnerabilities/",
	}
	VersionCheckVersionFlag = cli.StringFlag{
		Name:  "check.version",
		Usage: "Version to check",
		Value: fmt.Sprintf("G420/v%v/%v-%v/%v",
...
`,
	...
	versionCheckCommand = cli.Command{
		...
		Category:  "MISCELLANEOUS COMMANDS",
		Description: `
The version-check command fetches vulnerability-information from https://420integrated.com/420coin-overview/g420/g420-vulnerabilities/, 
...
func makecache(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: g420 makecache <block number> <outputdir>`)
	}
...
func makedag(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: g420 makedag <block number> <outputdir>`)
	}
	block, err := strconv.ParseUint(args[0], 0, 64)
	if err != nil {
		utils.Fatalf("Invalid block number: %v", err)
...
G420 is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

You should have received a copy of the GNU General Public License
along with g420. If not, see <http://www.gnu.org/licenses/>.`)
	return nil
}
