...
func (g *g420rpc) killAndWait() {
	g.g420.Kill()
	g.g420.WaitExit()
}

func (g *g420rpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.g420.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *g420rpc) addPeer(peer *g420rpc) {
	g.g420.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.g420.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.g420.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.g420.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.g420.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *g420rpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *g420rpc) waitSynced() {
	...
	select {
	case ev := <-ch:
		g.g420.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.g420.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.g420.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.g420.Fatalf("%v timeout syncing", g.name)
		break
	}
}
...
func startG420WithIpc(t *testing.T, name string, args ...string) *g420rpc {
	...
	var err error
	ipcpath := ipcEndpoint(ipcName, g.g420.Datadir)
...
func startLightServer(t *testing.T) *g420rpc {
	datadir := initG420(t)
	t.Logf("Importing keys to g420")
	runG420(t, "--nousb", "--datadir", datadir, "--password", "./testdata/password.txt", "account", "import", "./testdata/key.prv", "--lightkdf").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startG420WithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *g420rpc {
	datadir := initG420(t)
	return startG420WithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
