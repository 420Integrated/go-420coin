...
func (api *RetestfourtwentyAPI) mineBlock() error {
	...
	smokeLimit := core.CalcSmokeLimit(parent, 9223372036854775807, 9223372036854775807)
	header := &types.Header{
	...
	var blockFull = smokePool.Smoke() < params.TxSmoke
		...
		for nonce := statedb.GetNonce(address); ; nonce++ {
			if tx, ok := m[nonce]; ok {
				...
				receipt, err := core.ApplyTransaction(
					...
					smokePool,
					statedb,
					header, tx, &header.SmokeUsed, *api.blockchain.GetVMConfig(),
				)
				if err != nil {
				...
				txCount++
				if smokePool.Smoke() < params.TxSmoke {
					blockFull = true
					break
				}
			} else {
