...
type Conn struct {
	*rlpx.Conn
	...
	fourtwentyProtocolVersion uint
}
...
func (c *Conn) handshake(t *utesting.T) Message {
	...
	ourHandshake := &Hello{
		Version: 5,
		Caps: []p2p.Cap{
			{Name: "fourtwenty", Version: 64},
			{Name: "fourtwenty", Version: 65},
		},
		ID: pub0,
	}
	...
	case *Hello:
		...
		c.negotiateFourtwentyProtocol(msg.Caps)
		if c.fourtwentyProtocolVersion == 0 {
			t.Fatalf("unexpected fourtwenty protocol version")
		}
		return msg
	default:
...
func (c *Conn) negotiateFourtwentyProtocol(caps []p2p.Cap) {
	...
	for _, capability := range caps {
		if capability.Name != "fourtwenty" {
			continue
		}
		if capability.Version > highestFourtwentyVersion && capability.Version <= 65 {
			highestFourtwentyVersion = capability.Version
		}
	}
	c.fourtwentyProtocolVersion = highestFourtwentyVersion
}
...
loop:
	...
	// make sure fourtwenty protocol version is set for negotiation
	if c.fourtwentyProtocolVersion == 0 {
