...
func (pre *Prestate) Apply(vmConfig vm.Config, chainConfig *params.ChainConfig,
	...
	vmContext := vm.Context{
		...
		GetHash:     getHash,
		// SmokePrice and Origin needs to be set per transaction
	}
	// If DAO is supported/enabled, we need to handle it here. In g420 'proper', it's
	...
	for i, tx := range txs {
		...
		vmContext.SmokePrice = msg.SmokePrice()
		...
		// Create a new receipt for the transaction, storing the intermediate root and smoke used by the tx
		{
			...
			receipt := types.NewReceipt(root, msgResult.Failed(), smokeUsed)
...
func MakePreState(db 420db.Database, accounts core.GenesisAlloc) *state.StateDB {
