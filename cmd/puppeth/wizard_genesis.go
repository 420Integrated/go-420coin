...
// importGenesis imports a G420 genesis spec into puppeth.
...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-cppeth.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppeth and G420
		g420Json := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((g420Json), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", g420Json)

		// Export the genesis spec used by Cpp420 (formerly C++ 420coin)
		if spec, err := newCpp420GenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Cpp420 chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "cpp420", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly 420coinJ)
