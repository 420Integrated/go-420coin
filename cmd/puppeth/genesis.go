...
import (
	...
	math2 "github.com/420integrated/go-420coin/common/math"
...
type cpp420GenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newCpp420GenesisSpec converts a go-420coin genesis block into a Cpp420-specific
// chain specification format.
func newCpp420GenesisSpec(network string, genesis *core.Genesis) (*cpp420GenesisSpec, error) {
	// Only ethash is currently supported between go-420coin and cpp420
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Cpp420 format
	spec := &cpp420GenesisSpec{
		SealEngine: "Ethash",
	}
	// Some defaults
	...
	// config but cpp420 will sync with ETC clients up until the actual dao hard
	// fork block.
	...
	spec.Params.BlockReward = (*hexutil.Big)(ethash.SativaBlockReward)
	...
	spec.setPrecompile(1, &cpp420GenesisSpecBuiltin{Name: "ecrecover",
		Linear: &cpp420GenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &cpp420GenesisSpecBuiltin{Name: "sha256",
		Linear: &cpp420GenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &cpp420GenesisSpecBuiltin{Name: "ripemd160",
		Linear: &cpp420GenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &cpp420GenesisSpecBuiltin{Name: "identity",
		Linear: &cpp420GenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &cpp420GenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
		spec.setPrecompile(6, &cpp420GenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &cpp420GenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &cpp420GenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &cpp420GenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &cpp420GenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
	}
	if genesis.Config.IstanbulBlock != nil {
		...
		spec.setPrecompile(6, &cpp420GenesisSpecBuiltin{
			Name:          "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Cpp420 hardcoded the smoke policy
		spec.setPrecompile(7, &cpp420GenesisSpecBuiltin{
			Name:          "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Cpp420 hardcoded the smoke policy
		spec.setPrecompile(9, &cpp420GenesisSpecBuiltin{
...
func (spec *cpp420GenesisSpec) setPrecompile(address byte, data *cpp420GenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*cpp420GenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &cpp420GenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *cpp420GenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*cpp420GenesisSpecAccount)
	}
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-420coin/pull/11039,
...
// newParityChainSpec converts a go-420coin genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-420coin and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(ethash.SativaBlockReward)
...
// pyethcoinGenesisSpec represents the genesis specification format used by the
// Python 420coin implementation.
type pyethcoinGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	SmokeLimit   hexutil.Uint64    `json:"smokeLimit"`
...
// newPyethcoinGenesisSpec converts a go-420coin genesis block into a Parity specific
// chain specification format.
func newPyethcoinGenesisSpec(network string, genesis *core.Genesis) (*pyethcoinGenesisSpec, error) {
	// Only ethash is currently supported between go-420coin and pyethcoin
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyethcoinGenesisSpec{
