...
FROM fourtwentycoin/client-go:alltools-latest
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"fourtwentycoin listener port":       strconv.Itoa(info.node.port),
		"Funding amount (base tier)":   fmt.Sprintf("%d 420coins", info.amount),
