...
RUN \
	...
	sed -i 's/PuppethNetwork/{{.Network}}/g'     dist/js/420coinwallet-master.js && \
	sed -i 's/PuppethDenom/{{.Denom}}/g'         dist/js/420coinwallet-master.js && \
	sed -i 's/PuppethHost/{{.Host}}/g'           dist/js/420coinwallet-master.js && \
	sed -i 's/PuppethRPCPort/{{.RPCPort}}/g'     dist/js/420coinwallet-master.js
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["6174/tcp"]
