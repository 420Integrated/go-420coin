...
func (w *wizard) deployNode(boot bool) {
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 13013, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 13013, peersTotal: 50, peersLight: 0, smokeTarget: 7.5, smokeLimit: 10, smokePrice: 1}
		}
	}
	existed := err == nil
	...
	if !boot {
		...
		fmt.Println()
		fmt.Printf("What smoke price should the signer require (GWei)? (default = %0.3f)\n", infos.smokePrice)
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy 420coin node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
