...
type config struct {
	...
	fourtwentystats  string   // 420stats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // 420coin services known to be running on servers
...
// converts it to an 420coin address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by 420stats, which compares IPs textually, not by value.
