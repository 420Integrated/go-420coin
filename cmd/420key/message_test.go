...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "420key-test")
	if err != nil {
	...
	// Create the key.
	generate := run420key(t, "generate", "--lightkdf", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := run420key(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := run420key(t, "verifymessage", address, signature, message)
