...
import (
	...
	"github.com/420integrated/go-420coin/metrics/influxdb"
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `g420 account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func setMiner(ctx *cli.Context, cfg *miner.Config) {
	...
	if ctx.GlobalIsSet(LegacyMinerSmokeTargetFlag.Name) {
		cfg.SmokeFloor = ctx.GlobalUint64(LegacyMinerSmokeTargetFlag.Name)
		log.Warn("The flag --targetsmokelimit is deprecated and will be removed in the future, please use --miner.smoketarget")
	}
	...
	if ctx.GlobalIsSet(LegacyMinerSmokePriceFlag.Name) {
		cfg.SmokePrice = GlobalBig(ctx, LegacyMinerSmokePriceFlag.Name)
		log.Warn("The flag --smokeprice is deprecated and will be removed in the future, please use --miner.smokeprice")
	}
...
func SetFourtwentyConfig(ctx *cli.Context, stack *node.Node, cfg *fourtwenty.Config) {
	...
	if (ctx.GlobalIsSet(LegacyLightServFlag.Name) || ctx.GlobalIsSet(LightServeFlag.Name)) && ctx.GlobalIsSet(TxLookupLimitFlag.Name) {
		log.Warn("420Light server cannot serve old transaction status and cannot connect below les/4 protocol version if transaction lookup index is limited")
	}
	...
	if ctx.GlobalIsSet(NoDiscoverFlag.Name) {
		...
		if urls == "" {
			cfg.FourtwentyDiscoveryURLs = []string{}
		} else {
			cfg.FourtwentyDiscoveryURLs = SplitAndTrim(urls)
		}
	}
	...
	case ctx.GlobalBool(DeveloperFlag.Name):
		...
		if !ctx.GlobalIsSet(MinerSmokePriceFlag.Name) && !ctx.GlobalIsSet(LegacyMinerSmokePriceFlag.Name) {
...
func SetDNSDiscoveryDefaults(cfg *fourtwenty.Config, genesis common.Hash) {
	if cfg.FourtwentyDiscoveryURLs != nil {
	...
	if url := params.KnownDNSNetwork(genesis, protocol); url != "" {
		cfg.FourtwentyDiscoveryURLs = []string{url}
	}
	if cfg.SyncMode == downloader.SnapSync {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "g420.", tagsMap)
		}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) fourtwentydb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node, readOnly bool) (chain *core.BlockChain, chainDb fourtwentydb.Database) {
	var err error
	chainDb = MakeChainDatabase(ctx, stack)
...
// new format.
//
// e.g. g420 account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// g420 --keystore /tmp/mykeystore --lightkdf account new
//
