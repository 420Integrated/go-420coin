...
*For a more detailed introduction to checkpoint, please see the LES [spec](https://github.com/420coin/devp2p/blob/master/caps/les.md).*
...
```shell
go get github.com/420integrated/go-420coin/cmd/checkpoint-admin
```
...
It is worth noting that checkpoint-admin only supports clef as a signer for transactions and plain text(checkpoint). For more clef usage, please see the clef [tutorial](https://g420.420integrated.com/docs/clef/tutorial) .
...
*It is worth noting that the connected G420 node can be a fullnode or a light client. If it is fullnode, you must enable the LES protocol. E.G. add `--light.serv 50` to the startup command line flags*.
...
*CHECKPOINT_HASH is obtained based on this [calculation method](https://github.com/420integrated/go-420coin/blob/master/params/config.go#L251).*
...
Currently, only the 420coin mainnet and the default supported test network (ruderalis) activates this feature. If you want to activate this feature in your private network, you can overwrite the relevant checkpoint oracle settings through the configuration file after deploying the oracle contract.

* Get your node configuration file `g420 dumpconfig OTHER_COMMAND_LINE_OPTIONS > config.toml`
...
* Start g420 with the modified configuration file
