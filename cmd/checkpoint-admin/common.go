...
func newClient(ctx *cli.Context) *fourtwentyclient.Client {
	client, err := fourtwentyclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to 420coin node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, fourtwentyclient.NewClient(client))
	if err != nil {
