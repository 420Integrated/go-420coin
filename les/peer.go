...
// PeerInfo represents a short summary of the `fourtwenty` sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // 420coin network protocol version negotiated
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local 420coin node is running in archive mode, advertise ourselves we have
...
// participating in the Light 420coin sub-protocol.
