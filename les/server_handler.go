...
const (
	...
	fourtwentyVersion = 64              // equivalent fourtwenty version for the downloader
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light 420coin peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), forkID, h.forkFilter, h.server); err != nil {
		p.Log().Debug("Light 420coin handshake failed", "err", err)
		return err
	}
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light Fourtwentycoin peer rejected", "err", errFullClientPool)
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light 420coin response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light 420coin message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light 420coin message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	// accept returns an indicator if the request can be served.
