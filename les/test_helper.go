...
import (
	...
	"github.com/420integrated/go-420coin/p2p/nodestate"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			auth, _ := bind.NewKeyedTransactorWithChainID(bankKey, big.NewInt(13370))
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db fourtwentydb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db fourtwentydb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
