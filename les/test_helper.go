...
import (
	...
	"github.com/420integrated/go-420coin/accounts/abi/bind/backends"
	"github.com/420integrated/go-420coin/common"
	"github.com/420integrated/go-420coin/common/mclock"
	"github.com/420integrated/go-420coin/consensus/ethash"
	"github.com/420integrated/go-420coin/contracts/checkpointoracle/contract"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some 420coin to user1
		...
		case 1:
			...
			// bankUser transfers more 420coin to user1
		...
		case 2:
			// bankUser transfer some 420coin to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db 420db.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db 420db.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db 420db.Database, peers *clientPeerSet, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
