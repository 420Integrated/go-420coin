...
type nodeDB struct {
	...
	nbEvictCallBack func(mclock.AbsTime, negBalance) bool // Callback to determine if the negative balance can be evicted.
	clock           mclock.Clock
	closeCh         chan struct{}
...
// expireNodes iterates the whole node db and checks if the negative balance
// entry can deleted.
//
