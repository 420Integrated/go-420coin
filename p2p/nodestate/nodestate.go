...
func NewNodeStateMachine(db fourtwentydb.KeyValueStore, dbKey []byte, clock mclock.Clock, setup *Setup) *NodeStateMachine {
	if setup.flags == nil {
...
// disabled flags set.
// Note: this callback is not an operation callback but ForEach can be called from an
