...
import (
	...
	"github.com/420integrated/go-420coin/crypto/signify"
...
var (
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert 420coin contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
		...
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (420coin Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "g420",
			Description: "420coin CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "420coin private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "420coin account management tool.",
		},
	}
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 13 {
			...
			log.Println("go-420coin requires at least Go version 1.13 and cannot")
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with g420-release.pub"
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the go-420coin sources.
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "g420-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "420coin"
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "g420.exe" {
			g420Tool = file
		} else {
	...
	build.Render("build/nsis.geth.nsi", filepath.Join(*workdir, "g420.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("g420-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/420integrated/go-420coin/mobile")
	...
	archive := "g420-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "G420.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "G420.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "g420store/builds")`)
