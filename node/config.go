...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of g420 is "g420". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "G420", keep that.
	if name == "g420" || name == "g420-testnet" {
		name = "G420"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "g420" instances.
var isOldG420Resource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by g420 1.4 are used if they exist.
	if warn, isOld := isOldG420Resource[path]; isOld {
		oldpath := ""
		if c.name() == "g420" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldG420ResourceWarning, "Using deprecated resource file %s, please move this file to the 'g420' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-420coin-keystore")
		ephemeral = keydir
	}
