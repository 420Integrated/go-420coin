...
// testFourtwentyHandler is a mock event handler to listen for inbound network requests
// on the `eth` protocol and convert them into a more easily testable form.
type testFourtwentyHandler struct {
	blockBroadcasts event.Feed
	txAnnounces     event.Feed
	txBroadcasts    event.Feed
}

func (h *testFourtwentyHandler) Chain() *core.BlockChain              { panic("no backing chain") }
func (h *testFourtwentyHandler) StateBloom() *trie.SyncBloom          { panic("no backing state bloom") }
func (h *testFourtwentyHandler) TxPool() fourtwenty.TxPool                   { panic("no backing tx pool") }
func (h *testFourtwentyHandler) AcceptTxs() bool                      { return true }
func (h *testFourtwentyHandler) RunPeer(*fourtwenty.Peer, fourtwenty.Handler) error { panic("not used in tests") }
func (h *testFourtwentyHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testFourtwentyHandler) Handle(peer *fourtwenty.Peer, packet fourtwenty.Packet) error {
	switch packet := packet.(type) {
	case *fourtwenty.NewBlockPacket:
		h.blockBroadcasts.Send(packet.Block)
		return nil

	case *fourtwenty.NewPooledTransactionHashesPacket:
		h.txAnnounces.Send(([]common.Hash)(*packet))
		return nil

	case *fourtwenty.TransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil

	case *fourtwenty.PooledTransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil

	default:
		panic(fmt.Sprintf("unexpected fourtwenty packet type in tests: %T", packet))
	}
}
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		fourtwentyNoFork, _ = newHandler(&handlerConfig{
			Database:   dbNoFork,
			Chain:      chainNoFork,
		...
		fourtwentyProFork, _ = newHandler(&handlerConfig{
			Database:   dbProFork,
			Chain:      chainProFork,
			TxPool:     newTestTxPool(),
			Network:    1,
	...
	fourtwentyNoFork.Start(1000)
	fourtwentyProFork.Start(1000)

	// Clean up everything after ourselves
	defer chainNoFork.Stop()
	defer chainProFork.Stop()

	defer fourtwentyNoFork.Stop()
	defer fourtwentyProFork.Stop()
	...
	peerNoFork := fourtwenty.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork := fourtwenty.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- fourtwentyNoFork.runFourtwentyPeer(peerProFork, func(peer *fourtwenty.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- fourtwentyProFork.runFourtwentyPeer(peerNoFork, func(peer *fourtwenty.Peer) error { return nil })
	}(errc)
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := fourtwenty.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pSrc, handler.txpool)
	sink := fourtwenty.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runFourtwentyPeer(sink, func(peer *fourtwenty.Peer) error {
		return fourtwenty.Handle((*fourtwentyHandler)(handler.handler), peer)
	})
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runFourtwentyPeer(src, func(peer *fourtwenty.Peer) error {
		return fourtwenty.Handle((*fourtwentyHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	backend := new(testFourtwentyHandler)
	...
	go fourtwenty.Handle(backend, sink)
	...
	for len(seen) < len(insert) {
		...
		case 63, 64:
			select {
			case <-anns:
				t.Errorf("tx announce received on pre fourtwenty/65")
			case txs := <-bcasts:
				for _, tx := range txs {
		...
		case 65:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post fourtwenty/65")
			}
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := fourtwenty.NewPeer(protocol, p2p.NewPeer(enode.ID{byte(i)}, "", nil), sourcePipe, source.txpool)
		sinkPeer := fourtwenty.NewPeer(protocol, p2p.NewPeer(enode.ID{0}, "", nil), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runFourtwentyPeer(sourcePeer, func(peer *fourtwenty.Peer) error {
			return fourtwenty.Handle((*fourtwentyHandler)(source.handler), peer)
		})
		go sink.handler.runFourtwentyPeer(sinkPeer, func(peer *fourtwenty.Peer) error {
			return fourtwenty.Handle((*fourtwentyHandler)(sink.handler), peer)
		})
	}
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := fourtwenty.NewPeer(fourtwenty.FOURTWENTY64, p2p.NewPeer(enode.ID{1}, "", nil), p2pLocal, handler.txpool)
	remote := fourtwenty.NewPeer(fourtwenty.FOURTWENTY64, p2p.NewPeer(enode.ID{2}, "", nil), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()

	go handler.handler.runFourtwentyPeer(local, func(peer *fourtwenty.Peer) error {
		return fourtwenty.Handle((*fourtwentyHandler)(handler.handler), peer)
	})
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	sinks := make([]*testFourtwentyHandler, peers)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = new(testFourtwentyHandler)
	}
	// Interconnect all the sink handlers with the source handler
	var (
	...
	for i, sink := range sinks {
		...
		sourcePeer := fourtwenty.NewPeer(fourtwenty.FOURTWENTY64, p2p.NewPeer(enode.ID{byte(i)}, "", nil), sourcePipe, nil)
		sinkPeer := fourtwenty.NewPeer(fourtwenty.FOURTWENTY64, p2p.NewPeer(enode.ID{0}, "", nil), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
		...
		go fourtwenty.Handle(sink, sinkPeer)
	}
	// Subscribe to all the transaction pools
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := fourtwenty.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pSrc, source.txpool)
	sink := fourtwenty.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runFourtwentyPeer(src, func(peer *fourtwenty.Peer) error {
		return fourtwenty.Handle((*fourtwentyHandler)(source.handler), peer)
	})
