...
// support all the operations needed by the 420coin chain protocols.
...
type handlerConfig struct {
	Database   fourtwentydb.Database     // Database for direct sync insertions
...
// newHandler returns a handler for all 420coin chain management protocol.
...
// runFourtwentyPeer
func (h *handler) runFourtwentyPeer(peer *fourtwenty.Peer, handler fourtwenty.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("420coin network handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("420coin peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerFourtwentyPeer(peer); err != nil {
		peer.Log().Error("420coin peer registration failed", "err", err)
		return err
	}
...
func (h *handler) removePeer(id string) {
	...
	if fourtwenty != nil {
		...
		if err := h.peers.unregisterFourtwentyPeer(id); err != nil {
	...
	if fourtwenty != nil {
		fourtwenty.Peer.Disconnect(p2p.DiscUselessPeer)
	}
	if snap != nil {
...
func (h *handler) Stop() {
	...
	log.Info("420coin network protocol stopped")
}
...
func (h *handler) BroadcastBlock(block *types.Block, propagate bool) {
	hash := block.Hash()
	peers := h.peers.fourtwentyPeersWithoutBlock(hash)
...
func (h *handler) BroadcastTransactions(txs types.Transactions, propagate bool) {
	var (
		txset = make(map[*fourtwentyPeer][]common.Hash)
		annos = make(map[*fourtwentyPeer][]common.Hash)
	)
	...
	for peer, hashes := range annos {
		if peer.Version() >= fourtwenty.FOURTWENTY65 {
