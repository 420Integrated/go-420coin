...
// NewProtocolManager returns a new 420coin sub protocol manager. The 420coin sub protocol manages peers capable
// with the 420coin network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb 420db.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing 420coin peer", "peer", id)

	// Unregister the peer from the downloader and 420coin peer set
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("420coin protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("420coin peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("420coin handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p, pm.removePeer); err != nil {
		p.Log().Error("420coin peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("420coin message handling failed", "err", err)
			return err
		}
	}
}
...
type NodeInfo struct {
	Network    uint64              `json:"network"`    // 420coin network ID (420=Mainnet, Testnet=421)
