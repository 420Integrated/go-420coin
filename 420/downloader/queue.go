...
type fetchRequest struct {
	...
	From    uint64          // [420/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [420/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerHead      common.Hash                    // [420/62] Hash of the last queued header to verify order
	headerTaskPool  map[uint64]*types.Header       // [420/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [420/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [420/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [420/62] Currently pending header retrieval operations
	headerResults   []*types.Header                // [420/62] Result cache accumulating the completed headers
	headerProced    int                            // [420/62] Number of headers already processed from the results
	headerOffset    uint64                         // [420/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [420/62] Channel to notify when header download finishes

	// All data retrievals below are based on an already assembles header chain
	blockTaskPool  map[common.Hash]*types.Header // [420/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [420/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [420/62] Currently pending block (body) retrieval operations

	receiptTaskPool  map[common.Hash]*types.Header // [420/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [420/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [420/63] Currently pending receipt retrieval operations
