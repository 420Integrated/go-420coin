...
const (
	...
	// containing 200+ transactions, the practical limit will always
...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `fourtwenty` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `fourtwenty`.
...
// NodeInfo represents a short summary of the `fourtwenty` sub-protocol metadata
...
// nodeInfo retrieves some `fourtwenty` protocol metadata about the running host node.
...
// Handle is invoked whenever an `fourtwenty` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `fourtwenty`", "err", err)
			return err
		}
	}
}
...
func handleMessage(backend Backend, peer *Peer) error {
	...
	case msg.Code == NewPooledTransactionHashesMsg && peer.version >= FOURTWENTY65:
	...
	case msg.Code == GetPooledTransactionsMsg && peer.version >= FOURTWENTY65:
	...
	case msg.Code == TransactionsMsg || (msg.Code == PooledTransactionsMsg && peer.version >= FOURTWENTY65):
