...
const (
	...
	requestTimeout = 10 * time.Second // TODO: Make it dynamic like fast-sync.
...
type accountResponse struct {
	...
	nodes fourtwentydb.KeyValueStore // Database containing the reconstructed trie nodes
...
// Syncer is an 420integrated account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db    fourtwentydb.KeyValueStore // Database to store the trie nodes into (and dedup)
...
func NewSyncer(db fourtwentydb.KeyValueStore, bloom *trie.SyncBloom) *Syncer {
	return &Syncer{
		db:    db,
		bloom: bloom,
...
func (s *Syncer) OnStorage(peer *Peer, id uint64, hashes [][]common.Hash, slots [][][]byte, proof [][]byte) error {
	...
	var (
		dbs    = make([]fourtwentydb.KeyValueStore, len(hashes))
