...
func (pm *ProtocolManager) doSync(op *chainSyncOp) error {
	if op.mode == downloader.FastSync {
		...
		// the limit in the next fast sync(e.g. user kill G420 manually and
		// restart) then it will be hard for G420 to figure out the oldest block
