	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				...
				blockCtx := core.NewEVMBlockContext(task.block.Header(), api.fourtwenty.blockchain, nil)
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	blockCtx := core.NewEVMBlockContext(block.Header(), api.fourtwenty.blockchain, nil)
	for th := 0; th < threads; th++ {
		pend.Add(1)
		go func() {
			defer pend.Done()
...
func (api *PrivateDebugAPI) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	var (
		...
		vmctx       = core.NewEVMBlockContext(block.Header(), api.fourtwenty.blockchain, nil)
		canon       = true
	)
...
func (api *PrivateDebugAPI) TraceCall(ctx context.Context, args fourtwentyapi.CallArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceConfig) (interface{}, error) {
	...
	vmctx := core.NewEVMBlockContext(header, api.fourtwenty.blockchain, nil)
	return api.traceTx(ctx, msg, vmctx, statedb, config)
}
...
func (api *PrivateDebugAPI) computeTxEnv(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		txContext := core.NewEVMTxContext(msg)
		context := core.NewEVMBlockContext(block.Header(), api.fourtwenty.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, txContext, statedb, api.fourtwenty.blockchain.Config(), vm.Config{})
