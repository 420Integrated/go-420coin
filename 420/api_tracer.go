...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.fourtwenty.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.fourtwenty.blockchain.CurrentBlock()
	default:
		from = api.fourtwenty.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.fourtwenty.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.fourtwenty.blockchain.CurrentBlock()
	default:
		to = api.fourtwenty.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabaseWithCache(api.fourtwenty.ChainDb(), 16, "") // Chain tracing will probably start at genesis

	if number := start.NumberU64(); number > 0 {
		start = api.fourtwenty.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				signer := types.MakeSigner(api.fourtwenty.blockchain.Config(), task.block.Number())

				// Trace all the transactions contained within
				for i, tx := range task.block.Transactions() {
					msg, _ := tx.AsMessage(signer)
					vmctx := core.NewEVMContext(msg, task.block.Header(), api.fourtwenty.blockchain, nil)
					...
					task.statedb.Finalise(api.fourtwenty.blockchain.Config().IsEIP158(task.block.Number()))
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.fourtwenty.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.fourtwenty.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
			// Finalize the state so any modifications are written to the trie
			root, err := statedb.Commit(api.fourtwenty.blockchain.Config().IsEIP158(block.Number()))
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.fourtwenty.miner.PendingBlock()
	case rpc.LatestBlockNumber:
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewEVMContext(msg, block.Header(), api.fourtwenty.blockchain, nil)
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(vmctx, statedb, api.fourtwenty.blockchain.Config(), vm.Config{})
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	database := state.NewDatabaseWithCache(api.fourtwenty.ChainDb(), 16, "")

	for i := uint64(0); i < reexec; i++ {
		block = api.fourtwenty.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.fourtwenty.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceCall(ctx context.Context, args fourtwentyapi.CallArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceConfig) (interface{}, error) {
	// First try to retrieve the state
	statedb, header, err := api.fourtwenty.APIBackend.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	// Execute the trace
	msg := args.ToMessage(api.fourtwenty.APIBackend.RPCSmokeCap())
	vmctx := core.NewEVMContext(msg, header, api.fourtwenty.blockchain, nil)
	return api.traceTx(ctx, msg, vmctx, statedb, config)
}
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewEVM(vmctx, statedb, api.fourtwenty.blockchain.Config(), vm.Config{Debug: true, Tracer: tracer})
...
func (api *PrivateDebugAPI) computeTxEnv(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContext(msg, block.Header(), api.fourtwenty.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, statedb, api.fourtwenty.blockchain.Config(), vm.Config{})
