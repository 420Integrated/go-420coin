...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.420.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.420.blockchain.CurrentBlock()
	default:
		from = api.420.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.420.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.420.blockchain.CurrentBlock()
	default:
		to = api.420.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabaseWithCache(api.420.ChainDb(), 16, "") // Chain tracing will probably start at genesis

	if number := start.NumberU64(); number > 0 {
		start = api.420.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				signer := types.MakeSigner(api.420.blockchain.Config(), task.block.Number())

				// Trace all the transactions contained within
				for i, tx := range task.block.Transactions() {
					msg, _ := tx.AsMessage(signer)
					vmctx := core.NewEVMContext(msg, task.block.Header(), api.420.blockchain, nil)
					...
					task.statedb.Finalise(api.420.blockchain.Config().IsEIP158(task.block.Number()))
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.420.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.420.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
			// Finalize the state so any modifications are written to the trie
			root, err := statedb.Commit(api.420.blockchain.Config().IsEIP158(block.Number()))
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.420.miner.PendingBlock()
	case rpc.LatestBlockNumber:
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.420.engine.VerifyHeader(api.420.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.420.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		signer = types.MakeSigner(api.420.blockchain.Config(), block.Number())
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewEVMContext(msg, block.Header(), api.420.blockchain, nil)
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(vmctx, statedb, api.420.blockchain.Config(), vm.Config{})
...
func (api *PrivateDebugAPI) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		vmenv := vm.NewEVM(vmctx, statedb, api.420.blockchain.Config(), vmConf)
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	database := state.NewDatabaseWithCache(api.420.ChainDb(), 16, "")

	for i := uint64(0); i < reexec; i++ {
		block = api.420.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.420.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := rawdb.ReadTransaction(api.420.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) TraceCall(ctx context.Context, args 420api.CallArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceConfig) (interface{}, error) {
	...
	statedb, header, err := api.420.APIBackend.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	// Execute the trace
	msg := args.ToMessage(api.420.APIBackend.RPCSmokeCap())
	vmctx := core.NewEVMContext(msg, header, api.420.blockchain, nil)
	return api.traceTx(ctx, msg, vmctx, statedb, config)
}
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewEVM(vmctx, statedb, api.420.blockchain.Config(), vm.Config{Debug: true, Tracer: tracer})
	...
	case *vm.StructLogger:
		...
		return &420api.ExecutionResult{
			Smoke:         result.UsedSmoke,
			Failed:      result.Failed(),
			ReturnValue: returnVal,
			StructLogs:  420api.FormatLogs(tracer.StructLogs()),
		}, nil
...
func (api *PrivateDebugAPI) computeTxEnv(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContext(msg, block.Header(), api.420.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, statedb, api.420.blockchain.Config(), vm.Config{})
