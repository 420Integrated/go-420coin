...
func TestForkIDSplit(t *testing.T) {
	var (
		...
		420NoFork, _  = NewProtocolManager(configNoFork, nil, downloader.FullSync, 1, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, engine, chainNoFork, dbNoFork, 1, nil)
		420ProFork, _ = NewProtocolManager(configProFork, nil, downloader.FullSync, 1, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, engine, chainProFork, dbProFork, 1, nil)
	)
	420NoFork.Start(1000)
	420ProFork.Start(1000)
	...
	go func() { errc <- 420NoFork.handle(peerProFork) }()
	go func() { errc <- 420ProFork.handle(peerNoFork) }()
