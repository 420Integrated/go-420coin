...
func (p *statePrefetcher) Prefetch(block *types.Block, statedb *state.StateDB, cfg vm.Config, interrupt *uint32) {
	...
	for i, tx := range block.Transactions() {
		...
		if err := precacheTransaction(p.config, p.bc, nil, smokepool, statedb, header, tx, cfg); err != nil {
...
func precacheTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, smokepool *SmokePool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, cfg vm.Config) error {
	...
	_, err = ApplyMessage(vm, msg, smokepool)
	return err
}
