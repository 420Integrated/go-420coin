...
import (
	...
	"github.com/420integrated/go-420coin/crypto/blake2b"
	"github.com/420integrated/go-420coin/crypto/bls12381"
	"github.com/420integrated/go-420coin/crypto/bn256"
...
// PrecompiledContractsHomestead contains the default set of pre-compiled 420coin
...
// PrecompiledContractsByzantium contains the default set of pre-compiled 420coin
...
// PrecompiledContractsIstanbul contains the default set of pre-compiled 420coin
...
// PrecompiledContractsYoloV1 contains the default set of pre-compiled 420coin
...
func (c *bigModExp) RequiredSmoke(input []byte) uint64 {
	...
	switch {
	case smoke.Cmp(big64) <= 0:
		smoke.Mul(smoke, smoke)
	case smoke.Cmp(big1024) <= 0:
		smoke = new(big.Int).Add(
			new(big.Int).Div(new(big.Int).Mul(smoke, smoke), big4),
			new(big.Int).Sub(new(big.Int).Mul(big96, smoke), big3072),
		)
	default:
		smoke = new(big.Int).Add(
			new(big.Int).Div(new(big.Int).Mul(smoke, smoke), big16),
			new(big.Int).Sub(new(big.Int).Mul(big480, smoke), big199680),
		)
	}
	smoke.Mul(smoke, math.BigMax(adjExpLen, big1))
	smoke.Div(smoke, new(big.Int).SetUint64(params.ModExpQuadCoeffDiv))
