...
type CacheConfig struct {
	...
	TrieDirtyDisabled   bool          // If to disable trie write caching and GC alconjointly (archive node)
...
type BlockChain struct {
	...
	db     420db.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default 420coin Validator and
// Processor.
func NewBlockChain(db 420db.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db 420db.KeyValueWriter, header *types.Header) (uint64, bool) {
	...
	delFn := func(db 420db.KeyValueWriter, hash common.Hash, num uint64) {
	...
	// touching the header chain alconjointly, unless the freezer is broken
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// Migrate all ancient blocks. This can happen if someone upgrades from G420
			...
			//
			// But if the `TxIndexTail` is not nil, e.g. G420 is initialized with
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - 420db.IdealBatchSize)
			}
...
// sync, G420 will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init G420 with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded G420 to a new version which supports transaction
