...
// (i)   G420 restarts normally with valid legacy snapshot
// (ii)  G420 restarts normally with valid new-format snapshot
// (iii) G420 restarts after the crash, with broken legacy snapshot
// (iv)  G420 restarts after the crash, with broken new-format snapshot
// (v)   G420 restarts normally, but it's requested to be rewound to a lower point via SetHead
// (vi)  G420 restarts normally with a stale snapshot
type snapshotTest struct {
	legacy       bool   // Flag whether the loaded snapshot is in legacy format
	crash        bool   // Flag whether the g420 restarts from the previous crash
...
// Tests a G420 restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a G420 restart with valid but "legacy" snapshot. Before the shutdown,
...
// Tests a G420 was crashed and restarts with a broken snapshot. In this case the
...
// Tests a G420 was crashed and restarts with a broken snapshot. In this case
...
// Tests a G420 was crashed and restarts with a broken and "legacy format"
...
// Tests a G420 was running with snapshot enabled. Then restarts without
...
// Tests a G420 was running with leagcy snapshot enabled. Then restarts
...
// Tests the G420 was running with snapshot enabled and resetHead is applied.
...
// Tests the G420 was running with snapshot(legacy-format) enabled and resetHead
...
// Tests that g420 was running with snapshot(legacy-format) enabled and upgrades
// the disk layer journal(journal generator) to latest format. After that g420
