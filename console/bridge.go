...
type bridge struct {
	client   *rpc.Client         // RPC client to execute 420coin requests through
...
func getJ420(vm *goja.Runtime) *goja.Object {
	j420 := vm.Get("j420")
	if j420 == nil {
		panic(vm.ToValue("j420 object does not exist"))
	}
	return j420.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC method that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC method (saved in j420.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	newAccount, callable := goja.AssertFunction(getJ420(call.VM).Get("newAccount"))
	if !callable {
		return nil, fmt.Errorf("j420.newAccount is not callable")
	}
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	openWallet, callable := goja.AssertFunction(getJ420(call.VM).Get("openWallet"))
	if !callable {
		return nil, fmt.Errorf("j420.openWallet is not callable")
	}
...
// original RPC method (saved in j420.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	unlockAccount, callable := goja.AssertFunction(getJ420(call.VM).Get("unlockAccount"))
	if !callable {
		return nil, fmt.Errorf("j420.unlockAccount is not callable")
	}
...
// j420.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	// Send the request to the backend and return
	sign, callable := goja.AssertFunction(getJ420(call.VM).Get("sign"))
	if !callable {
		return nil, fmt.Errorf("j420.sign is not callable")
	}
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	if err := b.client.Call(&lastNumber, "fourtwenty_blockNumber"); err != nil {
		return nil, err
	}
	for time.Now().Before(deadline) {
		var number hexutil.Uint64
		if err := b.client.Call(&number, "fourtwenty_blockNumber"); err != nil {
			return nil, err
		}
